...
public class Layer 
{
	...
	{
		// Pour chaque neuron dans la couche actuelle on leur inject en entrée 
		// les outputs des neurons de la couche précédente
	...
	/**
	 * Ici, ce n'est qu'une simple somme de multiplication entre les synapses et
	 * les local error de la couche suivante. e1 = s2*e2 + s3*e3
	 * 
	 * @param forwardLayer
	 */
	public void computeLocalError(Layer forwardLayer) {
		// Pour chaque neuron de la couche actuelle, on calcul somme les erreurs de
		// la couche suivante
		...
		for (int i = 0; i < this.neurons.size(); i++) {
			...
			for (Neuron forwardNeuron : forwardLayer.neurons) {
				newLocalError +=
						forwardNeuron.getSynapses().get(i) * forwardNeuron.getLocalError();
			}
